...
SUPPORT_IMAGE_FORMATS = ['bmp', 'dib', 'jpeg', 'jpg', 'jpe', 'jp2', 'png', 'webp', 'pbm', 'pgm', 'ppm', 'pxm', 'pnm', 'pfm', 'sr', 'ras', 'tiff', 'tif', 'exr', 'hdr', 'pic']

def is_supported_format(filepath):
    ext = filepath if '.' not in filepath else os.path.splitext(filepath)[-1].lower()
...
def write_image(img, dir, prefix='', suffix='', ext='png'):
    if not os.path.exists(dir):
        raise Exception(f'Invalid directory: "{dir}" does not exist.')
    ...
    file_path = os.path.join(dir, f'{prefix}{"_" if prefix else ""}{unix_timestamp}{"_" if suffix else ""}{suffix}.{ext}')
